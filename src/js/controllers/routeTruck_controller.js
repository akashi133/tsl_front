...
export default class extends Controller {
   ...
   async connect() {
       if(global.user !== null) {
           ...
           let truckItemId = window.location.href;
           truckItemId = truckItemId.slice(truckItemId.indexOf('=') + 1);

           try {
               let truckItem = await api.GetTruck(truckItemId);
               ...
               for (let i = 0; i < truckItem.loadingpointsList.length; i++){
                   let city = await api.GetCity(truckItem.loadingpointsList[i].id);
                   pointsFrom.push(city.name + ',' + city.region.name +',' + city.country.name)
               }
               for (let i = 0; i < truckItem.unloadingpointsList.length; i++){
                   let city = await api.GetCity(truckItem.unloadingpointsList[i].id);
                   pointsTo.push(city.name + ',' + city.region.name +',' + city.country.name)
               }
               for (let i = 0; i < truckItem.additionalmailsList.length; i++){
                   emails.push(truckItem.additionalmailsList[i]);
               }
               for (let i = 0; i < truckItem.additionalphonesList.length; i++){
                   phones.push(truckItem.additionalphonesList[i]);
               }
               for (let i = 0; i < truckItem.additionList.length; i++){
                   addition.push(truckItem.additionList[i]);
               }
               for (let i = 0; i < truckItem.loadingtypeList.length; i++){
                   loadingtype.push(truckItem.loadingtypeList[i]);
               }
               for (let i = 0; i < truckItem.transporttypeList.length; i++){
                   transporttype.push(truckItem.transporttypeList[i]);
               }
               ...
               $('#cargo-item-data').html('' +
                   ...
                   '        <span class="cargo-data__text">'+ truckItem.createdAt + '</span>\n' +
                   '      </li>\n' +
                   '      <li>\n' +
                   ...
                   '        <span class="cargo-data__text">С '+ truckItem.from + ' по '+ truckItem.till + '</span>\n' +
                   '      </li>\n' +
                   '      <li>\n' +
                   ...
                   '        <span class="cargo-data__text">' +
                   'Вес: '+ truckItem.weight + '<br>' +
                   'Объем: '+ truckItem.volume + '<br>' +
