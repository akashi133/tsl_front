...
export default class extends Controller {
    ...
    addEmail() {
        ...
        $('.add-checkboxmail-box').append($('' +
            ...
            '<span class="delete-item delete-item--lg" title="Удалить дополнительный email" aria-label="Удалить дополнительный email" data-action="click->addingRoadside#deleteItem"></span>' +
            '</div>' +
            ''));
    }
    ...
    addTel() {
        ...
        $('.add-tel-box').append($('' +
            ...
            '                        </div>\n' +
            '                      </div>\n' +
            '<span class="delete-item delete-item--lg" title="Удалить дополнительный телефон" aria-label="Удалить дополнительный телефон" data-action="click->addingRoadside#deleteItem"></span>' +
            '</div>'));
    ...
    addPhoto() {
        ...
        $('#add-photo-list').append(
            ...
            '   </span>\n' +
            '   <span class="delete-link" title="Удалить" data-action="click->addingRoadside#deletePhoto">Удалить</span>\n' +
            '</div>');
    ...
    async addAdvert() {
        ...
        if (form.checkValidity()) {
            ...
            try {
                ...
                let result = await api.CreateRoadsideService(json);
            ...
            $('.adding-avto-sale__container').prepend(
                '<label class="col-sm-12 col-md-6 mb-4 addpoint-plus">\n' +
                '              <input class="adding-form__btn" id="form-adding__addadvert" type="button" value="+" title="Добавить еще объявление" data-action="click->addingRoadside#reloadpage">\n' +
