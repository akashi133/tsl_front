/*
* для формы объявления редактиварование запчасти
*/
...
export default class extends Controller {
    ...
    async editService() {
        ...
        try {
            ...
            let spare = await api.GetSparePart(advIndex);


            $('#editSpareForm').prepend('' +
                '<input type="hidden" value="' + advIndex +'" name="id">');

            this.myTransporttype = spare.transporttypesList;
            this.myCategoriesList = spare.categoriesList;

            $('#title').val(spare.title);

            let $textarea = $('#text-counter');
            $textarea.val(spare.description);
            $('#counter').html(spare.description.length);

            this.myImagesList = spare.imagesList;
            this.myImagesList.forEach((image)=>{
                this.addPhoto(image)
            });

            if(spare.location !== undefined) {
                this.serviceCity();
                let city = await api.GetCity(spare.location.id);
                ...
                $('#addressStreet').val(spare.location.address);
            } else {
                this.serviceCity();
            }
            ...
            myEmails.forEach(function(item) {
                ...
                $('#new-email-box').append('' +
                    ...
                    '<span class="delete-item delete-item--lg" title="Удалить дополнительный email" aria-label="Удалить дополнительный email" data-action="click->lk-spare-edit#deleteItem"></span>' +
                    '</div>');
            });

            let myPhones = [];
            let myPhonesCounter = 1;
            myPhones = spare.phoneList;
            myPhones.shift();
            myPhones.forEach(function (item){
                $('#new-tel-box').append('' +
                    ...
                    '<span class="delete-item delete-item--lg" title="Удалить дополнительный телефон" aria-label="Удалить дополнительный телефон" data-action="click->lk-spare-edit#deleteItem"></span>' +
                    '</div>')
            })
    ...
    addPhoto(src = "") {
        ...
        $('#add-photo-list').append(
            ...
            '   </span>\n' +
            '   <span class="delete-link" title="Удалить" data-action="click->lk-spare-edit#deletePhoto">Удалить</span>\n' +
            '</div>');
    ...
    async addAdvert() {
        ...
        if (form.checkValidity()) {
            ...
            try {
                ...
                let result = await api.UpdateSparePart(json);
    ...
    async  connect() {
        ...
        api.FindReference(0).then(res =>{
            ...
            this.myTransporttype.forEach(function (item){
                $('#transport >option').filter(function (){
                    if ($(this).text() === item){
