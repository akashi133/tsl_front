/*
* для формы добавления объявления об ГРУЗ
*/
...
export default class extends Controller {
    ...
    addPointLoading() {
        ...
        $('.addpoint-loadingbox').append($('' +
            ...
            ' </div>\n' +
            '<span class="delete-item" title="Удалить точку" aria-label="Удалить точку" data-action="click->addingCargo#deleteItem"></span>' +
            '</div>'));
    ...
    addEmail() {
        ...
        $('.add-checkboxmail-box').append($('' +
            ...
            '<span class="delete-item delete-item--lg" title="Удалить дополнительный email" aria-label="Удалить дополнительный email" data-action="click->addingCargo#deleteItem"></span>' +
            '</div>' +
            ''));
    }
    ...
    addTel() {
        ...
        $('.add-tel-box').append($('' +
            ...
            '                        </div>\n' +
            '                      </div>\n' +
            '<span class="delete-item delete-item--lg" title="Удалить дополнительный телефон" aria-label="Удалить дополнительный телефон" data-action="click->addingCargo#deleteItem"></span>' +
            '</div>'));
   ...
   async addAdvert() {
        ...
        if (form.checkValidity()) {
            ...
            try {
                ...
                let result = await api.CreateCargo(json);
            ...
            $('#addingCargoForm, .form-adding__text').slideUp('slow', function(){
                $(this).remove();
            });
            $('.container__adding-avto').prepend(
                '<label class="col-sm-12 col-md-6 mb-4 addpoint-plus">\n' +
                '              <input class="adding-form__btn" id="form-adding__addadvert" type="button" value="+" title="Добавить еще объявление" data-action="click->addingCargo#reloadpage">\n' +
