...
export default class extends Controller {
    ...
    async editCargo(){
        ...
        try {
            let cargo = await api.GetCargo(advIndex);

            $('#editCargoForm').prepend('' +
                '<input type="hidden" value="' + advIndex +'" name="id">');


            $('#shipmentdate-from').val(cargo.from);
            $('#shipmentdate-until').val(cargo.till);
            ...
            {
                if ($(this).val() === cargo.currency)
                {
                    $(this).prop("selected", true);
                }
            });

            this.myTransportType = cargo.transporttypeList[0];
            this.myCargoType = cargo.cargotype;
            this.myTypeLoad = cargo.loadingtypeList;
            this.myReference = cargo.additionList;

            for (const item of cargo.loadingpointsList) {
            ...
            for (const item of cargo.unloadingpointsList) {
                let city = await api.GetCity(item.id);
            ...
            this.myEmails = cargo.additionalmailsList;
            this.myEmails.shift();
            this.myEmails.forEach((item)=>{
                ...
                $('.add-checkboxmail-box').append($('' +
                    ...
                    '<span class="delete-item delete-item--lg" title="Удалить дополнительный email" aria-label="Удалить дополнительный email" data-action="click->lk-cargo-edit#deleteItem"></span>' +
                    '</div>' +
                    ''));
            });

            //в глобальную переменную ставим массив телефонов
            this.myPhones = cargo.additionalphonesList;
            this.myPhones.shift();
            console.debug(this.myPhones);
            this.myPhones.forEach((item)=>{
                ...
                $('#default-tel-box').append($('' +
                    ...
                    '<span class="delete-item delete-item--lg" title="Удалить дополнительный телефон" aria-label="Удалить дополнительный телефон" data-action="click->lk-cargo-edit#deleteItem"></span>' +
                    '</div>' +
                    ''));
                });
    ...
    addPointLoading() {
        ...
        $('.addpoint-loadingbox').append($('' +
            ...
            ' </div>\n' +
            '<span class="delete-item" title="Удалить точку" aria-label="Удалить точку" data-action="click->lk-cargo-edit#deleteItem"></span>' +
            '</div>'));
   ...
   async addAdvert() {
        ...
        if (form.checkValidity()) {
            ...
            try {
                ...
                let result = await api.UpdateCargo(json);
...
/*            $('.container__adding-avto').prepend(
   ...
   /* transportTypeChange() {
       let transportType = $('#transport').val();
       if(transportType === 'автобус' || transportType === 'микроавтобус') {
            $('#cargoType').val('61');
...
/*        this.addPointLoading();*/
       ...
       if(global.user.role === 'admin'){
        ...
        api.FindReference(0).then(res =>{
            ...
            /* тип груза */
            api.FindReference(9).then(res =>{
                res.referenceList.forEach(item =>{
                    $('#cargoType').append('<option value="' + item.id + '">' + item.name + '</option>')
                });
               /* this.transportTypeChange();*/

                let mySelectOpt = this.myCargoType;
                $("#cargoType >option").filter( function()
                {
                    if ($(this).text() === mySelectOpt)
                    {
                        $(this).prop("selected", true);
                    }
                });
