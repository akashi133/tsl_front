 ...
 /*   fuelType(){
    ...
    checkType(){
        $('input:radio[name="autoType"]').change(function(){
            ...
            console.debug(checkAutoType);

            /* бренд  авто\автобус\спецтехника */
            api.FindReference(checkBrand).then(res =>{
                $('#brand option').remove();
                console.debug(res);
            ...
            /* тип транспорта */
            api.FindReference(checkAutoType).then(res =>{
...
/*    transmission(){
    ...
    addEmail() {
        ...
        $('.add-checkboxmail-box').append($('' +
            ...
            '<span class="delete-item delete-item--lg" title="Удалить дополнительный email" aria-label="Удалить дополнительный email" data-action="click->addingAvtoSale#deleteItem"></span>' +
            '</div>' +
            ''));
    }
    ...
    addTel() {
        ...
        $('.add-tel-box').append($('' +
            ...
            '                        </div>\n' +
            '                      </div>\n' +
            '<span class="delete-item delete-item--lg" title="Удалить дополнительный телефон" aria-label="Удалить дополнительный телефон" data-action="click->addingAvtoSale#deleteItem"></span>' +
            '</div>'));
    ...
    addPhoto() {
        ...
        $('#add-photo-list').append(
            ...
            '   </span>\n' +
            '   <span class="delete-link" title="Удалить" data-action="click->addingAvtoSale#deletePhoto">Удалить</span>\n' +
            '</div>');
 ...
 async addAdvert() {
        ...
        if (form.checkValidity()) {
            ...
            /*    await new Promise(r => setTimeout(r, 20000));*/
                let result = await api.CreateTransport(json);
            ...
            $('.container__adding-avto').prepend(
                ...
                '              <input class="adding-form__btn" id="form-adding__addadvert" type="button" value="+" title="Добавить еще объявление" data-action="click->addingAvtoSale#reloadpage">\n' +
